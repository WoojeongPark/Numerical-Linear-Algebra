...
q=zeros(n-1,1); q(1)=1; %Initial approximated eigenvector q
prev_q=q;
T=A-eye(n-1); %Since A is a symmetric positive-semidefinite matrix and we already know the smallest eigenvalue is 0.
%Just to check it, we choose \mu=1 by using Gerschgorin's circle theorem.
for iter=1:max_iter
    z=T\prev_q; 
    q=z/norm(z);
    diff_v=norm(q-prev_q);
